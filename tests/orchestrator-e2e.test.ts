...
describe("runOrchestration with SDK (happy path)", () => {
  ...
  it("resumes in-progress state when pre-plan read was empty but state.json exists before init", async () => {
    const config = singleTaskConfig();
    const runId = "run-post-plan-reread";
    const inProgress = createInitialState(config, runId);
    inProgress.status = "running";
    inProgress.started_at = "2026-01-01T00:00:00.000Z";
    const { store: baseStore, files } = createInMemoryRepoStore({ "config.yaml": toYaml(config) });
    let stateReadCount = 0;
    const store = {
      ...baseStore,
      async readFile(rid: string, filename: string): Promise<string> {
        if (filename === "state.json") {
          stateReadCount += 1;
          if (stateReadCount === 1) return "";
          return serialize(inProgress);
        }
        return baseStore.readFile(rid, filename);
      },
    } as unknown as RepoStoreClient;
    const fake = new FakeAgentClient({
      defaultScripts: [completedWorkerScript("t1", runId)],
    ...
    expect(finalState.started_at).toBe("2026-01-01T00:00:00.000Z");
    expect(finalState.status).toBe("completed");
  });

  it("skips orchestration when stopped state appears after empty pre-plan read", async () => {
    const config = promptOnlyConfig();
    const runId = "run-stopped-post-plan-reread";
    const stoppedState = createInitialState(config, runId);
    stoppedState.status = "stopped";
    ...
    const { store: baseStore } = createInMemoryRepoStore({
      "config.yaml": toYaml(config),
