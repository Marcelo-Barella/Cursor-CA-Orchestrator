...
describe("reconcileInFlightLaunchesFromEvents", () => {
  ...
  it.each([
    { status: "pending" as const, agentId: "agent-existing" },
    { status: "running" as const, agentId: "agent-kept" },
  ])("skips agents that already have an agent_id when $status", ({ status, agentId }) => {
    const config = createConfig(["t1"]);
    const state = createInitialState(config, "run-recover-skip");
    state.agents.t1!.agent_id = agentId;
    state.agents.t1!.status = status;
    const events = [
      taskLaunchedEvent({
    ...
    expect(reconcileInFlightLaunchesFromEvents(state, events)).toBe(false);
    expect(state.agents.t1!.agent_id).toBe(agentId);
    expect(state.agents.t1!.status).toBe(status);
  });

  it.each([
    { detail: "Launched t1", runId: "run-recover-blank-detail" },
    { detail: "Launched t1 ()", runId: "run-recover-blank" },
  ])("ignores task_launched events with blank agent_id when detail is $detail", ({ detail, runId }) => {
    const config = createConfig(["t1"]);
