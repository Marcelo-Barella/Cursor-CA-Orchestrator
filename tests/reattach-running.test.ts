...
describe("reattachWorkers running tasks", () => {
  ...
  it("reattaches launching workers with agent_id in state without relaunching", async () => {
    const config = singleTaskConfig();
  ...
  it("reattaches to the latest task_launched agent when duplicate launch events exist", async () => {
    const config = singleTaskConfig();
    const runId = "run-recover-duplicate-launch";
    const staleAgentId = "agent-stale";
    const liveAgentId = "agent-live-latest";
    const state = createInitialState(config, runId);
    state.status = "running";
    state.started_at = new Date().toISOString();
    seedMainAgent(state, { agent_id: "orch-1", status: "running", started_at: state.started_at });
    ...
    const firstLaunch = JSON.stringify({
      ...
      agent_node_id: "t1",
      agent_kind: "task",
      detail: `Launched t1 (${staleAgentId})`,
      payload: { agent_id: staleAgentId, run_id: "run-stale" },
    });
    const secondLaunch = JSON.stringify({
      ...
      payload: { agent_id: liveAgentId, run_id: "run-live" },
    });

    const { store, files } = createInMemoryRepoStore({
      "config.yaml": toYaml(config),
      "state.json": serialize(state),
      "events.jsonl": `${firstLaunch}\n${secondLaunch}\n`,
    });
  ...
  it("resumes an active run after transient state.json read failures", async () => {
    const config = singleTaskConfig();
    const runId = "run-active-transient";
    ...
    const store = {
      ...baseStore,
      async readFile(runIdArg: string, filename: string): Promise<string> {
        ...
        return baseStore.readFile(runIdArg, filename);
      },
    } as unknown as typeof baseStore;

    await runOrchestration(runId, fake, store);

    expect(stateReadCount).toBeGreaterThan(2);
    const final = JSON.parse(files.get("state.json")!);
    expect(final.status).toBe("completed");
    expect(fake.launches).toHaveLength(0);
  });
