...
import type { ModelSelectionConfig, OrchestratorConfig, TaskConfig } from "./config/types.js";
...
export function readTaskFailureRetryCap(): number {
  ...
  if (raw !== undefined && raw !== "") {
    const n = Number(raw);
    if (Number.isFinite(n) && n >= 0 && n <= TASK_FAILURE_MAX_RETRIES_CAP) {
      return Math.floor(n);
    }
  }
...
async function handleTaskFailureWithOptionalRetry(
  ctx: LoopContext,
  taskId: string,
  appendTaskFailedEvent: () => Promise<void>,
  terminal: { finishedAt: string; summary: string },
): Promise<"retried" | "terminal"> {
  const agent = ctx.state.agents[taskId]!;
  await appendTaskFailedEvent();
  const cap = readTaskFailureRetryCap();
  if (agent.retry_count < cap) {
    agent.retry_count += 1;
    ...
    await appendEvent(
      ctx.repoStore,
      ctx.runId,
      makeEvent("task_retried", `Task ${taskId} retried (failure retry ${agent.retry_count})`, taskId, {
        payload: { retry_count: String(agent.retry_count) },
      }),
    );
    markStateDirty(ctx);
    return "retried";
...
async function runWorkerStream(ctx: LoopContext, handle: WorkerHandle, streamLaunch: WorkerStreamInfo): Promise<void> {
  ...
  try {
    ...
    const gitHead = awaited.git?.branches?.[0];
    result = {
      status: awaited.status,
      durationMs: awaited.durationMs,
      git: gitHead != null ? { branch: gitHead.branch, prUrl: gitHead.prUrl } : undefined,
      model: awaited.model?.id,
  ...
  if (streamLaunch.runLine && streamLaunch.planRef) {
    const ownerRepo = parseGithubOwnerRepo(streamLaunch.repoUrl);
    if (ownerRepo) {
      const gk = groupKeyForRepo(streamLaunch.repoUrl, streamLaunch.planRef);
      ctx.state.repo_run_head = ctx.state.repo_run_head ?? {};
      ctx.state.repo_run_head[gk] = runBranchName(ctx.config.target.branch_prefix, ctx.runId, streamLaunch.planRef);
  ...
  if (payloadStatus === "blocked") {
    ...
    const summaryLine = payloadSummary ?? errText ?? "Worker agent errored";
    await handleTaskFailureWithOptionalRetry(
      ctx,
      taskId,
      () =>
        appendEvent(
          ...
          makeEvent("task_failed", `Task ${taskId} failed${errText ? `: ${errText}` : ""}`, taskId, {
            payload: errText
              ? { error: errText, payload_source: payloadSource, last_status: String(lastStatus ?? "") }
              : { payload_source: payloadSource, last_status: String(lastStatus ?? "") },
          }),
        ),
      { finishedAt: finalizedAt, summary: summaryLine },
    );
...
async function retryBlockedAgent(ctx: LoopContext, agent: AgentState): Promise<void> {
  ...
  const streamLaunch = task ? await resolveWorkerStreamInfo(ctx, task) : EMPTY_WORKER_STREAM_INFO;
  nextHandle.done = runWorkerStream(ctx, nextHandle, streamLaunch);
...
async function runPlanningPhase(
  ...
  await applyTaskPlanContent(config, runId, repoStore, planContent, bootstrapUrl);
}
...
async function reattachWorkers(ctx: LoopContext): Promise<void> {
  for (const [taskId, agent] of Object.entries(ctx.state.agents)) {
    ...
    try {
      ...
      const streamLaunch = task ? await resolveWorkerStreamInfo(ctx, task) : EMPTY_WORKER_STREAM_INFO;
      handle.done = runWorkerStream(ctx, handle, streamLaunch);
      ctx.activeWorkers.set(taskId, handle);
    } catch (err) {
      agent.status = "failed";
    ...
    if (planContent) {
      try {
        ...
        planningOutcome = {
          status: "finished",
          emitStarted: false,
          detail: `Planning completed: ${taskCount} tasks (reused existing plan)`,
        };
      } catch (reuseErr) {
        planningOutcome = { status: "failed", detail: String(reuseErr) };
      }
    }
    if (!planningOutcome) {
      try {
        ...
        planningOutcome = {
          status: "finished",
          emitStarted: true,
          detail: `Planning completed: ${config.tasks.length} tasks`,
        };
      } catch (planningExc) {
        planningOutcome = { status: "failed", detail: String(planningExc) };
      }
    }
  }
  ...
  if (planningOutcome) {
    ...
    if (planningOutcome.status === "finished") {
      ...
      await appendEvent(
        repoStore,
        runId,
        makeEvent("planning_completed", planningOutcome.detail, null, { phase_id: "planning", agent_kind: "phase" }),
      );
    } else {
      await appendEvent(
        repoStore,
        runId,
        makeEvent("planning_failed", planningOutcome.detail, null, { phase_id: "planning", agent_kind: "phase" }),
      );
    }
    await syncToRepo(repoStore, runId, state);
