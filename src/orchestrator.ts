...
async function retryBlockedAgent(ctx: LoopContext, agent: AgentState): Promise<void> {
  ...
  if (existingHandle) {
    ...
    try {
      ...
      console.error(
        `Blocked retry resume failed for task ${agent.task_id}, re-launching: ${error instanceof Error ? error.message : String(error)}`,
      );
      ...
      await appendEvent(
        ctx.repoStore,
        ctx.runId,
        makeEvent("task_retried", `Task ${agent.task_id} retried (re-launched)`, agent.task_id),
      );
      markStateDirty(ctx);
      return;
    }
...
async function reattachWorkers(ctx: LoopContext): Promise<void> {
  ...
  for (const [taskId, agent] of Object.entries(ctx.state.agents)) {
    ...
    try {
      ...
      if (!reattachRun) {
        if (resetEventRecoveredForRelaunch(ctx, taskId, agent)) {
          await safeDisposeAgent(sdkAgent);
          continue;
        }
